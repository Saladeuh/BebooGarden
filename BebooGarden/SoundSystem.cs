...
internal class SoundSystem
{
  ...
  {
    Sound sound = AmbiSounds[Game1.Instance.Random.Next(AmbiSounds.Count())];
    Channel channel = System.PlaySound(sound, paused: false)!;
    channel.Set3DAttributes(new Vector3(Game1.Instance.Random.Next(-20, 20), Game1.Instance.Random.Next(-20, 20), 5f), default, default);
    channel.Set3DMinMaxDistance(0, 35);
    bool enabled = _ambiTimer.Enabled;
    _ambiTimer?.Dispose();
    _ambiTimer = new Timer(Game1.Instance.Random.Next(4000, 8000));
    _ambiTimer.Elapsed += onAmbiTimer;
  ...
  {
    //Game1.Instance.GameWindow?.DisableInput();
    ...
    //Game1.Instance.GameWindow?.EnableInput();
  }

  internal void PlaySadMusic()
  {
    MusicTransition(Game1.Instance.SoundSystem.SadMusicStream, 464375, 4471817, TimeUnit.PCM, 0.1f);
  }
